...
import { projectsApi, tasksApi, deploymentsApi, secretsApi } from '../services/api';
import { subscribeToUptime } from '../services/socketService';
...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Sync projects with Backend API on mount
  useEffect(() => {

    async function loadProjectsFromBackend() {
      try {
        setIsLoading(true);
        const remoteProjects = await projectsApi.getAll();
        if (remoteProjects && remoteProjects.length > 0) {
          setProjects(remoteProjects);
          localStorage.setItem('nexify_devops_projects', JSON.stringify(remoteProjects));
        }
      } catch (err) {
        console.warn('[ProjectContext] Using local cache/seed data as backend is initializing:', err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // Listen for real-time WebSocket Uptime & Telemetry updates
  useEffect(() => {
    const unsubscribe = subscribeToUptime((pingData) => {
      setProjects((prev) =>
        prev.map((p) =>
          p.id === pingData.projectId
            ? {
                ...p,
                latencyMs: pingData.latencyMs,
                health: pingData.health || p.health,
              }
            : p
        )
      );
  ...
  const addProject = (projectData: Omit<ClientProject, 'id' | 'createdAt' | 'updatedAt' | 'tasks' | 'deployments' | 'docs' | 'secrets' | 'team'>): ClientProject => {
    ...
    projectsApi.create(newProject).catch((e) => console.warn('[Backend Sync Error] addProject:', e));
  ...
  const updateProject = (id: string, updates: Partial<ClientProject>) => {
    ...
    projectsApi.update(id, updates).catch((e) => console.warn('[Backend Sync Error] updateProject:', e));
  };
  ...
  const deleteProject = (id: string) => {
    ...
    projectsApi.delete(id).catch((e) => console.warn('[Backend Sync Error] deleteProject:', e));
  };
  ...
  const addTask = (projectId: string, task: Omit<TaskItem, 'id' | 'createdAt'>) => {
    ...
    tasksApi.add(projectId, newTask).catch((e) => console.warn('[Backend Sync Error] addTask:', e));
  };
  ...
  const updateTaskStatus = (projectId: string, taskId: string, status: TaskItem['status']) => {
    ...
    tasksApi.update(projectId, taskId, { status }).catch((e) => console.warn('[Backend Sync Error] updateTask:', e));
  };
  ...
  const addDeployment = (projectId: string, deploy: Omit<DeploymentRecord, 'id' | 'timestamp'>) => {
    ...
    deploymentsApi.add(projectId, newDeploy).catch((e) => console.warn('[Backend Sync Error] addDeployment:', e));
  };
  ...
  const deleteDeployment = (projectId: string, deploymentId: string) => {
    ...
    deploymentsApi.delete(projectId, deploymentId).catch((e) => console.warn('[Backend Sync Error] deleteDeployment:', e));
  };
  ...
  const addSecret = (projectId: string, secret: Omit<ProjectSecret, 'id' | 'updatedAt'>) => {
    ...
    secretsApi.add(projectId, {
      key: secret.key,
      value: secret.value,
      environment: secret.environment,
      description: secret.description,
      masked: secret.masked,
    }).catch((e) => console.warn('[Backend Sync Error] addSecret:', e));
  };
  ...
  const deleteSecret = (projectId: string, secretId: string) => {
    ...
    secretsApi.delete(projectId, secretId).catch((e) => console.warn('[Backend Sync Error] deleteSecret:', e));
  };
  ...
  const addDomain = (projectId: string, domain: string) => {
    ...
    projectsApi.addDomain(projectId, cleanDomain).catch((e) => console.warn('[Backend Sync Error] addDomain:', e));
  };
  ...
  const removeDomain = (projectId: string, domain: string) => {
    ...
    projectsApi.removeDomain(projectId, domain).catch((e) => console.warn('[Backend Sync Error] removeDomain:', e));
  };
