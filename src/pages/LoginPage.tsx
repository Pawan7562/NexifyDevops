...
export const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authApi.login(developerEmail, accessKey, securityPin);
      if (response.success && response.token) {
        localStorage.setItem('nexify_dev_token', response.token);
        if (response.user) {
      ...
      console.warn('[Backend Auth] API login failed, checking fallback credentials:', err.response?.data?.error || err.message);
      
      // If server returned a specific auth rejection message
